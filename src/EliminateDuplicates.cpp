    ...
    while (nSortedSegs > 1) {//while there are segments to merge
        ...
        for (int m = 0; m <= passMerges; m++) {
            ...
            while (segmentsLeft != 0) {//while all of the segments have not been merged.
                ...
                int minBlockIndex = i;
                //compares records with minRec. Edv isws prepei na kanw min heap.
                ...
                if (!(nSortedSegs <= memSize)) {
                    (*minBlock).entries[(*minBlock).nreserved++] = minRec;
                }
                /******ELIMINATE DUBLICATES*********/
                else {
                    if(add2){//calls it once so we have initial value to minRec.
                        addUn = minRec;
                        add2=false;
                    }
                    if (add) {
                        (*minBlock).entries[(*minBlock).nreserved++] = addUn;
                        (*nunique) += 1;
                        addUn = minRec;
                        add = false;
                    } else {
                        if (compareRecords(addUn, minRec, field) != 0) {
                            addUn = minRec;
                            add = true;
                        }
                    }
